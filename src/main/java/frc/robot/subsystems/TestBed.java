// Copyright (c) FIRST and other WPILib contributors.
// Open Source Software; you can modify and/or share it under the terms of
// the WPILib BSD license file in the root directory of this project.

package frc.robot.subsystems;

import com.revrobotics.CANSparkMax;
import com.revrobotics.CANSparkMaxLowLevel.MotorType;

import edu.wpi.first.wpilibj2.command.SubsystemBase;
import frc.robot.Constants;

public class TestBed extends SubsystemBase {

  // Motor for testing mechanisms
  private static final CANSparkMax testMotor = new CANSparkMax(Constants.TEST_MOTOR_PORT, MotorType.kBrushless);

  /** Creates a new TestBed. */
  public TestBed() {
    // Inverts the motor
    testMotor.setInverted(Constants.TEST_MOTOR_INVERSION);
  }

  /**
   * Moves the motor using joystick values so we take doubles as parameters
   * @author Kenneth Wong
   */
  public void moveMotor(double speed) {
    testMotor.set(speed);
  }

  /**
   * Stops the motor
   * @author Kenneth Wong
   */
  public void stopMotor() {
    testMotor.set(0);
  }

  @Override
  public void periodic() {
    // This method will be called once per scheduler run
  }
}
